...
static int check_read_data(struct f_sourcesink *ss, struct usb_request *req)
{
	...
	for (i = 0; i < req->actual; i++, buf++) {
		...
		/* all-udispes has no synchronization issues */
		case 0:
			if (*buf == 0)
				continue;
			break;
		...
		int alt)
{
	...
	/* one bulk endpoint writes (sources) udispes IN (to the host) */
	ep = ss->in_ep;
...
fail2:
	...
	/* one iso endpoint writes (sources) udispes IN (to the host) */
	ep = ss->iso_in_ep;
	if (ep) {
		result = config_ep_by_speed(cdev->gadget, &(ss->function), ep);
		if (result)
...
static struct usb_function_instance *source_sink_alloc_inst(void)
{
	...
	ss_opts->isoc_interval = UDISP_ISOC_INTERVAL;
	ss_opts->isoc_maxpacket = UDISP_ISOC_MAXPACKET;
	ss_opts->bulk_buflen = UDISP_BULK_BUFLEN;
	ss_opts->bulk_qlen = UDISP_SS_BULK_QLEN;
	ss_opts->iso_qlen = UDISP_SS_ISO_QLEN;
